...
import by.goncharov.task05.service.creators.ThreadsWithPhaserCreator;
import by.goncharov.task05.view.Locale;
import java.io.IOException;
import java.util.List;
import java.util.concurrent.TimeUnit;

public class ThreadsWithPhaserCommand implements Command {
    @Override
    public String execute(List<String> list) throws InterruptedException, IOException, DAOException {
        ...
        StringBuilder result = new StringBuilder().append(Locale.
                getInstance().getResourceBundle().getString("FOURTH")).append(
                new GetIntroduction().get(matrix)
        );
        List<Integer> listForThreads = new ListForThreadsDAO().getList();
        new PutNumbersInMainDiagonal().execute(new ThreadsWithPhaserCreator().create(listForThreads, matrix));
